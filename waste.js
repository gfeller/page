...
const classLabels = {
  ...
  // Define max no. of detections per image.
  ...
  async function detect(videoElement) {
    ...
    async function processFrame() {
      ...
      let input = tf.image.resizeBilinear(tf.browser.fromPixels(frame), [
        448,
        448
      ]);
  ...
  function inferenceResults(boxes, classes, scores, n, frame) {
    ...
    for (let i = 0; i < numDetectionsToShow; i++) {
      ...
      // Adjust the bounding box coordinates according to screen size.
      const y_min = Math.max(0, Math.floor(boundingBox[0] * frame.height));
      const y_max = Math.min(
        frame.height,
        Math.floor(boundingBox[2] * frame.height)
      );
      const x_min = Math.max(0, Math.floor(boundingBox[1] * frame.width));
      const x_max = Math.min(
        frame.width,
        Math.floor(boundingBox[3] * frame.width)
      );
